...
//   const chartData: ChartDataHourly[] | null | undefined = forecastData.map(
//     (data) => ({
//       time: data.time,
//       temp: data.temp,
//     })
...
import type { WeatherData, Units, ChartDataHourly } from "../../types";
import {
  iconUrlFromCode,
  formatTemperature,
...
export function ForecastHourly({
  ...
  if (!weather?.forecast_time) return <div>Loading...</div>;

  const forecastData = weather.forecast_time.slice(0, 5);

  const chartData: ChartDataHourly[] = forecastData.map((d) => ({
    time: d.time,
    temp: d.temp,
  }));
  ...
  return (
    <Panel title="3 hour step forecast" className="overflow-hidden">
